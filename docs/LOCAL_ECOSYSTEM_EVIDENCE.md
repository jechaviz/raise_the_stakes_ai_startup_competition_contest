...
| VImport | Provides extraction, redaction, local vault refs, contracts, and evidence-friendly source handling. | Active with `profile-manifest`, `profile-application-packet`, `profile-secure-inputs`, `shape`, and jobapply contracts. | Use redacted manifests before any AI shaping. |
...
v run . -- profile-application-packet evidence\public\redacted_source_manifest.json evidence\public\vimport_profile_application_packet.json --opportunity raise_the_stakes
v run . -- profile-secure-inputs evidence\public\secure_inputs.public_schema.json --opportunity raise_the_stakes
...
C:\git\v\v.exe -o $env:TEMP\veloclaw_raise.exe run . policy-check --capability "inspect:shape@vimport[source=yo_redacted]"
```
...
The current receipt is `evidence/public/veloclaw_profile_policy_check.txt`.
